...
# NOTE THAT :
# - if your input is /home/data/tokenized/cat_dir/dog_dir/file_to_study.txt,
# the algorithm consider that the directories data,tokenized,cat_dir,dog_dir already exist. 
# - the output root will then have to be /home/data/output/cat_dir/dog_dir
# THIS ALGORITHM RECREATE THE FOLDER AND SUBFOLDER TREE STARTING AT your input_root,
###### END EXAMPLES ######
...
import matplotlib
matplotlib.use('Agg') #necessary to work with matplotlib on your terminal 
...
def K_core_function(root,file_name,root_output,pos_tag_to_keep,lower,w):
    ...
    if(not len(full_text)==0):
        ...
        if(file_name==None):
            ...
            name_doc_dens="{}/keywords_dens_w:{}_POS:{}.xls".format(os.path.join(root_output,file_name),w,'-'.join(pos_tag_to_keep))
            name_png_dens="{}/subgraph_dens_w:{}_POS:{}.png".format(os.path.join(root_output,file_name),w,'-'.join(pos_tag_to_keep))
            name_doc_inf="{}/keywords_inf_w:{}_POS:{}.xls".format(os.path.join(root_output,file_name),w,'-'.join(pos_tag_to_keep))
            name_png_inf="{}/subgraph_inf_w:{}_POS:{}.png".format(os.path.join(root_output,file_name),w,'-'.join(pos_tag_to_keep))
        ...
        print("--- The file {} has been processed and results are saved in {}\n".format(os.path.join(root,file_name),os.path.join(root_output,file_name)))
...
if(os.path.isdir(input_root)):
    ...
    for root,dirs,files in os.walk(input_root):
        ...
        if(results_for_each_file): 
            for f in files:
                if(not os.path.exists(os.path.join(root_output_full,f))):
                    os.makedirs(os.path.join(root_output_full,f))
                nb_done+=1
    ...
    f=input_root.split('/')[-1]
    root='/'.join(input_root.split('/')[:-1])
    if(not os.path.exists(os.path.join(root_output,f))):
        os.makedirs(os.path.join(root_output,f))
    K_core_function(root,f,root_output,pos_tag_to_keep,lower,w)
